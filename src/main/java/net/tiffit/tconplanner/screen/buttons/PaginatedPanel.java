...
public class PaginatedPanel<T extends AbstractWidget> extends PlannerPanel {
    ...
    public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float p_230430_4_) {
        ...
        if(totalPages > 1) {
            ...
            int page = parent.getCacheValue(cachePrefix + ".page", 0);
            guiGraphics.fill(scrollX, getY(), scrollX + 3, getY() + height, 0x0f_ffffff + (isHovered ? 0x0a_000000 : 0));
            guiGraphics.fill(scrollX, getY() + (int)(scrollPageHeight*page), scrollX + 3, getY() + (int)(scrollPageHeight*(page+rows)), 0x0f_ffffff + (isHovered ? 0x0f_000000 : 0));
        }
    }
