...
public class SliderWidget extends AbstractWidget {
    ...
    public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        for(int dx = getX() - 2; dx < getX() + width + 2; dx++){
        	guiGraphics.blit(PlannerScreen.TEXTURE, dx, center - 2, 176, 78, 1, 4);
        }
        int sliderX = getX() + (int)(width*percent);
        guiGraphics.blit(PlannerScreen.TEXTURE, sliderX - 2, getY(), 178, 78, 4, 20);
        Font font = Minecraft.getInstance().font;
        int minValSize = font.width(min + "");
        guiGraphics.drawString(font, min + "", getX() - minValSize - 5, getY() + 6, 0xff_ff_ff_ff);
        guiGraphics.drawString(font, max + "", getX() + width + 5, getY() + 6, 0xff_ff_ff_ff);
        guiGraphics.drawCenteredString(font, value + "", sliderX, getY() + 22, 0xff_ff_ff_ff);
    }
